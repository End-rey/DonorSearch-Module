...
@auth_router.message(F.content_type == "web_app_data", isAuth(), whatWebApp("Добавить донацию"))
...
@auth_router.message(F.text == "Мои напоминания", isAuth())
async def my_notifications_handler(message: types.Message, session: AsyncSession) -> None:
    donations = await donation_orm_handlers.get_donations_from_now(session, user_id=auth_user.user_id)
    if donations:
...
@auth_router.message(F.text == "Список центров сдачи крови")
async def bloodcentres_handler(message: types.Message, session: AsyncSession) -> None:
    ...
    if bloodcentres:
        ...
        await message.answer("Что-то пошло не так =(, повторите позже", reply_markup=types.ReplyKeyboardRemove())
...
@auth_router.message(F.text == "Указать город", StateFilter(None))
...
@auth_router.message(Sity.waiting_for_sity)
async def handle_sity(message: types.Message, session: AsyncSession, state: FSMContext) -> None:
    ...
    if city:
        ...
        await bloodcentres_handler(message, session)
    else:
        await message.answer("Не получилось =(, повторите позже", reply_markup=types.ReplyKeyboardRemove())
    await state.clear()


@auth_router.message(F.content_type == types.ContentType.LOCATION)
